...
- **`/process-transcript`** — General transcript analysis: extracts actions, reference material, project context, decisions, insights. Proposes an integration plan before acting. *Role fit:* The "plan → confirm → execute" shape that keeps Partner work transparent and reversible.
- **`/process-work-transcript`** — Work-specific transcript variant with meeting-type detection, cross-team coordination extraction, and work-domain context loading. *Role fit:* Specialized subclass for the work-context use case.
